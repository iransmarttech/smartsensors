...
def data_api(request):
    ...
    try:
        ...
        # Historical data (last 50 records for each sensor)
        # تاریخچه air_quality با زمان جدا
        air_quality_history = AirQuality.objects.order_by('-timestamp')[:50]
        air_quality_list = []
        
        for record in air_quality_history:
            air_quality_list.append({
                ...
                # زمان با ثانیه جداگانه
                'timestamp': record.get_date_time(),  # تاریخ و زمان کامل: "2025-12-02 10:01:45"
                'time_with_seconds': record.get_time_with_seconds(),  # فقط زمان با ثانیه: "10:01:45"
                'time_only': record.get_time_only(),  # فقط ساعت و دقیقه: "10:01"
            })

        # برای بقیه سنسورها هم همین کار رو می‌کنیم
        mr007_list = []
        for record in MR007.objects.order_by('-timestamp')[:50]:
            mr007_list.append({
                'voltage': record.voltage,
                'rawValue': record.rawValue,
                'lel_concentration': record.lel_concentration,
                'timestamp': record.get_date_time(),
                'time_with_seconds': record.get_time_with_seconds(),
            })

        me4_so2_list = []
        for record in ME4SO2.objects.order_by('-timestamp')[:50]:
            me4_so2_list.append({
                'voltage': record.voltage,
                'rawValue': record.rawValue,
                'current_ua': record.current_ua,
                'so2_concentration': record.so2_concentration,
                'timestamp': record.get_date_time(),
                'time_with_seconds': record.get_time_with_seconds(),
            })

        ze40_list = []
        for record in ZE40.objects.order_by('-timestamp')[:50]:
            ze40_list.append({
                'tvoc_ppb': record.tvoc_ppb,
                'tvoc_ppm': record.tvoc_ppm,
                'dac_voltage': record.dac_voltage,
                'dac_ppm': record.dac_ppm,
                'uart_data_valid': record.uart_data_valid,
                'analog_data_valid': record.analog_data_valid,
                'timestamp': record.get_date_time(),
                'time_with_seconds': record.get_time_with_seconds(),
            })

        device_info_list = []
        for record in DeviceInfo.objects.order_by('-timestamp')[:50]:
        ...
        history = {
            'air_quality': air_quality_list,
            'mr007': mr007_list,
            'me4_so2': me4_so2_list,
            'ze40': ze40_list,
        ...
        result = {
            'air_quality': aq and {
                ...
                'temperature': aq.temperature,
                'humidity': aq.humidity,
                'timestamp': aq.get_date_time() if aq.timestamp else None,
                'time_with_seconds': aq.get_time_with_seconds() if aq.timestamp else None,
                'time_only': aq.get_time_only() if aq.timestamp else None,
            },
            'mr007': mr and {
                'voltage': mr.voltage,
                'rawValue': mr.rawValue,
                'lel_concentration': mr.lel_concentration,
                'timestamp': mr.get_date_time() if mr.timestamp else None,
                'time_with_seconds': mr.get_time_with_seconds() if mr.timestamp else None,
            },
            'me4_so2': me4 and {
                'voltage': me4.voltage,
                'rawValue': me4.rawValue,
                'current_ua': me4.current_ua,
                'so2_concentration': me4.so2_concentration,
                'timestamp': me4.get_date_time() if me4.timestamp else None,
                'time_with_seconds': me4.get_time_with_seconds() if me4.timestamp else None,
            },
            'ze40': ze40 and {
                'tvoc_ppb': ze40.tvoc_ppb,
                'tvoc_ppm': ze40.tvoc_ppm,
                ...
                'uart_data_valid': ze40.uart_data_valid,
                'analog_data_valid': ze40.analog_data_valid,
                'timestamp': ze40.get_date_time() if ze40.timestamp else None,
                'time_with_seconds': ze40.get_time_with_seconds() if ze40.timestamp else None,
            },
            'ip_address': info.ip_address if info else None,
            'network_mode': info.network_mode if info else None,
            'history': history,
